...
def parseArgs():
	parser = ArgumentParser(description='Extracts the top two best taxa '
		...
		'global match value', epilog='NOTE: there is a strict 10 MB maximum '
		'JSON size to extract with this fast lxml module, unlike the bs4 '
		'module, due to its dependency on the C++ libxml2 which was '
		'introduced at least in 2.7.3: Jan 18 2009 (source: '
		'http://xmlsoft.org/news.html)', add_help=False)
...
def main():
	...
	with open(ifh, encoding='utf-8') as html_file:
		html_data = lxml.etree.HTML(html_file.read(),
			parser=lxml.etree.HTMLParser(encoding='utf-8'))
		if opt.outfile is not None:
			ofh = os.path.abspath(os.path.expanduser(opt.outfile))

	json_data = None
	for element in html_data.iter():
